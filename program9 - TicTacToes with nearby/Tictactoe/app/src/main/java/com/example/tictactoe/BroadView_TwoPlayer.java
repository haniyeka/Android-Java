...
public class BroadView_TwoPlayer extends View{
    ...
    private TwoplayersActivity twoplayersActivity;

    public BroadView_TwoPlayer(Context context) {
        super(context);
    }

    public BroadView_TwoPlayer(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        gridPaint = new Paint();
        gridPaint.setColor(Color.GRAY);
        oPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        oPaint.setColor(Color.parseColor("#0097A7"));
    ...
    public void setActivity(TwoplayersActivity a) {
        twoplayersActivity = a;
    }

    public void setGame(TictactoeTwoPlayer g) {
        tictactoeTwoPlayer = g;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!tictactoeTwoPlayer.isGameEnded()  &&  event.getAction() == MotionEvent.ACTION_DOWN) {
    ...
    private void drawBoard(Canvas canvas, TictactoeTwoPlayer gameEngine) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
